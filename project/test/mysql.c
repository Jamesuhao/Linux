 ...
 * 1.初始化客户端
 *  (1)初始化操作句柄  mysql_init 
 *  (2)连接服务器      mysql_real_connect 
 *  (3)设置字符集      mysql_set_character_set 
 * 2.执行语句
 *  (1)执行语句        mysql_query 
 *  (2)获取结果        mysql_store_result 
 *  (3)遍历结果        mysql_num_rows/mysql_num_fields/mysql_fetch_row 
 * 3.关闭数据库，释放资源 mysql_close 
 * 4.执行每个函数的过程中获取上一次错误原因
 */
#include<stdio.h>
#include<unistd.h>
#include<stdlib.h>
...
int main()
{
  ...
  //连接服务器：mysql_real_connect(操作句柄，主机地址，用户名，密码，数据库名称，端口，套接字文件，标志)
  ...
  const char* sql_str="select * from tb_dishes;";
  //执行语句
