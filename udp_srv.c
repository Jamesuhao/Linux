...
#include<string.h>
#include<unistd.h>
#include<netinet/in.h>//sockaddt结构体、IPPRPTP_UDP都在这个头文件中
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=3)
  {
    printf("./udp_srv ip port em：./unp_srv 192.168.122.132 9000\n");
    return -1;
  }
  ...
  //绑定地址信息bind(套接字描述符，地址结构，地址长度)；
  //struct sockaddr_in是一个ipv4的地址结构
  //struct in_addr{uint32_t s_addr};ip地址
  struct sockaddr_in addr;
  addr.sin_family = AF_INET;
  //htons-将两个字节数的主机字节序整数转换为网络字节序的整数
  addr.sin_port = htons(port_addr);//千万不能使用htonl
  //将一个点分十进制的字符串IP地址转换为网络字节序的整数IP地址
  addr.sin_addr.s_addr = inet_addr(ip_addr);
  socklen_t len = sizeof(struct sockaddr_in);//获取IPV4地址长度
  ...
  while(1)
  {
    ...
    memset(buf,0x00,1024);//情空buf中的数据
    scanf("%s",buf);
